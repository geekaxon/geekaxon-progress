...
> **Launch scope = build-steps 1–39 (COMPLETE). Phase 4 (40–44) is the usability/polish scope; the specced build order currently ends at 44.** Any further capability is added as a new, fully-authored `specs/NN-slug.md` (45+) only when its requirements are defined — never improvised. A missing spec is always a hard stop (`[HUMAN_REQUIRED]`), never a cue to guess.
