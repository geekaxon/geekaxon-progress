...
- **Last completed build-step:** **2 — Foundation: Tenancy + RLS + Domain Resolver.** ✅ DONE/APPROVED (2026-07-03) — `Tenant`/`Branch` models (snake_case `@map`), migration with reusable `apply_tenant_rls()` SQL fn (ENABLE+FORCE RLS, fail-closed `current_setting('app.tenant_id',true)` policy) applied to `branches`; real `runWithTenant` (tx + `set_config` LOCAL, `assertTenantId`); ALS tenant-context + host→tenant resolver (dormant custom-domain) + tenant-scoped guard/middleware; idempotent Ganatra seed (+deploy.sh step). **Isolation proven in-CI via pglite running the actual migration SQL.** Gates green (typecheck 16/16, lint 10/10, build 10/10, jest: db 15/15 + api 14/14).
- **Current build-step:** **3 — Auth.** Read `specs/03-auth.md` (authoritative). If missing/empty, STOP with `[HUMAN_REQUIRED]`.
- **Next-step pointer:** → `specs/03-auth.md`
- **Branch state:** `feat/02-tenancy-rls` complete (awaiting controller merge to `staging`), stacked on `feat/01-foundation-monorepo`. Step 3 branch `feat/03-auth` stacks on it if not yet merged. (Repo must be cloned onto the server ONCE before the first deploy — see ARCHITECTURE.md §15.)
