...
- **Last completed build-step:** **none** (greenfield).
- **Current build-step:** **1 — Foundation: Monorepo + DB + Prisma base + health endpoint + deploy.sh + CI.** Read `specs/01-foundation-monorepo.md` (authoritative). If missing/empty, STOP with `[HUMAN_REQUIRED]`.
- **Next-step pointer:** → `specs/01-foundation-monorepo.md`
- **Branch state:** none yet. First branch `feat/01-foundation-monorepo` off the initial commit; controller merges it to `staging` at the checkpoint. (Repo must be cloned onto the server ONCE before the first deploy — see ARCHITECTURE.md §15.)

### Recent steps (one-liners — FULL detail in PROGRESS-HISTORY.md)
- _(none yet — build has not started. The first entry will be build-step 1 once complete.)_
