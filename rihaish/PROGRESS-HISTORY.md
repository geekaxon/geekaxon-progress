...
## 79 — p2-cleanup — DONE (2026-07-16)

Maintenance pass clearing the six smaller confirmed defects from the code review (spec 79). Branch `feature/79-p2-cleanup`. Lint + typecheck green; unit/integration/e2e left to the controller gates.

1. **Dashboard MRR shown as raw paisa.** `lib/platform/dashboard.ts` downcast the BigInt MRR to `Number` and the page rendered it via `Intl.NumberFormat` (unit-less integer). Changed `DashboardData.mrrMinor` to `bigint`, pass `BigInt(billing.mrrMinor)` (kept BigInt to the boundary), and render it on `app/[locale]/platform/page.tsx` with `formatMoney(data.mrrMinor, "PKR", 0, uiLocale)` — same helper as the billing page.

2. **Meter tariff math in floating point.** `lib/meters/rules.ts` accumulated `band * rateMinor` in a JS number. Rewrote `computeAmountMinor` to run entirely in BigInt: consumption is scaled to integer milli-units (`UNIT_SCALE = 1000n`), slab/flat amounts accumulate as minor×scale, rounded to whole minor exactly once at the end. `TariffSlab.rateMinor` is now `bigint`; added `parseSlabs(raw)` to coerce the persisted JSON (numbers) at every DB-read boundary (`billing.ts`, `service.ts` TariffRow map). The wire `TariffDto.slabs` keeps `rateMinor: number` (JSON has no BigInt). New tests: fractional multi-slab consumption (150.5 units) and 3.333-unit flat rate assert exact BigInt results; `parseSlabs` coercion covered. Integration-test slab amounts unchanged.

3. **Two divergent TOTP-enrolment UIs unified.** Extracted the sign-in `EnrolStep`'s QR-first UI into one shared client component `components/platform/totp-enrol-card.tsx` (logo + QR + collapsible manual key + one-time recovery codes with a save-confirmation). Both the sign-in flow (`platform-sign-in.tsx`, wired to `/api/auth/login/enrol` with the challenge token) and the forced first-login gate (`platform-enrol-totp.tsx`, wired to `/api/platform/totp/enrol`) are now thin wrappers over it — the forced gate previously showed only the base32 secret, no QR/logo. The console enrol route (`app/api/platform/totp/enrol/route.ts`) now verifies with `matchTotpCounter` (replay-counter guard, advancing `totpLastCounter`) instead of `verifyTotp`, returns a server-rendered `qr` on start, and mints the ten one-time recovery codes (hashed, stale ones cleared) on completion — matching the login enrol path. No new i18n keys (all already present from the sign-in flow); `secretLabel`/`success` keys left in place (no orphan-key test).

4. **CI ran all 48 e2e specs despite the "12 critical / 6-min" claim.** `playwright.config.ts` gained a `critical` project whose `testMatch` is an explicit, documented list of the 12 critical spec files (spec 62's table, mapped to shipped filenames); the existing `chromium` project stays the full suite. New script `test:e2e:critical` sets `E2E_CRITICAL=1` (arms the 6-min `globalTimeout`) and runs `--project=critical`; `test:e2e` now pins `--project=chromium`. `ci.yml`: every push/PR runs the 12 critical specs; the `staging` branch and a new nightly cron (`0 3 * * *`) run the full suite. A red run still blocks the deploy either way.

5. **Page-guard latent hole.** `lib/platform/page-guard.ts` resolved a `pendingTotpEnrolment` (no `totpVerifiedAt`) session as a platform actor, unlike the API guard. Split into `getPlatformOperator` (identity without the TOTP gate — mirrors `requirePlatformOperator`) and `getPlatformActor` (now rejects any pending OR non-TOTP-verified session, mirroring `requirePlatform`). The layout (`app/[locale]/platform/layout.tsx`) uses the operator to decide which gate to render (sign-in vs forced enrolment) and the stricter actor for the shell, so a data-reading page never sees a half-authed session.

6. **Dead code.** Removed `assertPlatformSignup` from `lib/signup/service.ts` (0 callers — each /signup route already rejects a non-platform host inline) plus its now-unused `HostContext` import and test block. `callers.ts` and the pricing modules deleted by spec 76 are already gone; the remaining `lib/pricing/*` modules are still imported (not dead).
